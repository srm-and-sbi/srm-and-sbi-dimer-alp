...
ROLE. An objective measure of how realistic the simulated imaging is: it quantifies, as a scalar with
a significance test, how far the experimental recordings sit from the synthetic videos in the trained
detector embedding. Because the detector workflow marginalizes the biology as a nuisance, the
estimator's embedding is a representation of the imaging parameters, so a distance measured in it is a
statement about IMAGING realism — not biology. It is the detector-side counterpart of the mirror
analysis the canonical workflow would run (imaging marginalized, biology represented), which reads a
biological discrepancy instead. Design and justification: DETECTOR_WORKFLOW.md, section "Quantitative
experimental-versus-synthetic distance"; usage and interpretation:
SRM_AND_SBI_DIMER_ALP_DETECTOR_Embedding_Space_Distance.md (beside this script).

WHERE TO RUN. On a machine with a capable GPU: it embeds videos through the trained Complex3DCNN, so
it is a GPU step (rcl01 or an HPC GPU node). It is a post-hoc, user-driven analysis in
Script_Bank/Analysis — NOT a canonical pipeline stage and never wired into the stage dispatcher; it is
complementary to, and run separately from, the Detector Experiment stage. It is self-contained: given
the trained estimator and the generated/experimental videos it needs nothing else.

WHAT IT NEEDS.
  - the trained detector estimator (the version-portable artifact from the Detector Inference stage);
  - the held-out synthetic EVAL video sets (the same EVAL split the estimator was evaluated on);
  - the experimental recordings (the .tif files under the experiment subdirectory);
  - the run's timing (`--total-time-seconds`), which locates all of the above and names the outputs;
  - optionally, the EVAL theta sets (the imaging parameters that generated those videos) — only for
    the PC1 parameter-tracking add-on; the distance measure runs without them.

WHAT IT DOES.
  - embeds the experimental recordings (diced to model-length windows) and the synthetic EVAL videos
    through the estimator's Complex3DCNN;
  - computes two two-sample statistics on the raw embeddings — Maximum Mean Discrepancy (MMD) and a
    Classifier Two-Sample Test (C2ST) — with recording-level (cell) significance, so correlated
    windows from one recording are not counted as independent;
  - reports them for the pooled experimental set against the synthetic set (the primary
    imaging-realism number, since the imaging is a property of the microscope, not the biological
    condition) and, as diagnostics, per condition and between conditions;
  - writes a report and deterministic figures — the distance distributions, the C2ST scores, the
    inter-group distance matrix, and a PCA/classical-MDS projection with a Shepard faithfulness
    panel — that put all groups on shared axes;
  - when the EVAL imaging parameters are present, additionally ranks which imaging parameter the
    embedding's dominant axis (PC1) tracks (Spearman), naming the direction the embedding varies
    along.

WHAT IT DOES NOT DO.
  - It does NOT close a gap. It only measures. A measured gap is reduced by changing the model
    (recalibrating the imaging or widening the imaging prior) or the training (an embedding-alignment
    term) — separate work, not performed here.
  - It does NOT recover ground truth. The recordings have none; this measures distributional overlap,
    not a demonstrated recovery of true parameters (recovery is quantified only on held-out synthetic
    data in the Detector Evaluation stage).
  - It does NOT answer the biological question or discriminate imaging from biology. In the detector
    embedding the biology is marginalized, so the distance is imaging realism; the biological
    discrimination is the canonical workflow's mirror companion.
  - It does NOT use, build, or require the `Nuisance_DLI`, does NOT modify the estimator, and produces
    NO calibration artifact — only a diagnostic report.

Reads  <data_bank>/<experiment_subdir>/Experiment_<KIND>_Cell_<n>_<span>S_RAW.tif   (experimental)
       <data_bank>/<video>/<alias>_{timing}_Video_Set_TASK_{k}_EVAL.zarr           (synthetic EVAL)
       <data_bank>/<posit>/<alias>_{timing}_Estimator.npz                          (required estimator)
       <data_bank>/<theta>/<alias>_{timing}_Theta_Set_TASK_{k}_EVAL.zarr           (optional: PC1 tracking)
Writes <data_bank>/<posit>/<alias>_{timing}_Embedding_Space_Distance/              (report + figures)

Usage:
    MACHINE_PROFILE=<p> python .../SRM_AND_SBI_DIMER_ALP_DETECTOR_Embedding_Space_Distance.py \\
...
from srm_and_sbi_dimer_alp import detector_embedding_space_distance as esd
...
# Cap per group when computing the O(n^2) pairwise-distance figures; a subsample estimates the
# distance distributions faithfully while bounding cost. Not a scientific knob — a rendering detail.
...
# Concise report-facing descriptions of the imaging parameters — the six learnable targets plus the
# five SCOPE camera-nuisance parameters — distilled from the NOTE fields in
# detector_parameterization.DETECTOR_PARAMETERIZATION / DETECTOR_NUISANCE_SCOPE (the authoritative long-form source).
_PARAM_MEANING = {
...
def _load_posterior(R):
    ...
    posterior = artifacts.load_estimator(str(R["estimator_path"]), device=str(device),
                                          expected_parameter_keys=det.DETECTOR_PARAMETER_KEYS)
...
def _load_eval_theta(R, n_tasks):
    ...
    ``DETECTOR_PARAMETER_KEYS``, or ``(None, None, None)`` if any task's theta set is absent -- in
    ...
    keys = det.DETECTOR_PARAMETER_KEYS
    labmap = {e["KEY"]: (e.get("LABEL") or e["KEY"]) for e in det.DETECTOR_PARAMETERIZATION}
...
def _track_collapse(pc1, theta, keys, labels):
    """Rank each imaging parameter by |Spearman correlation| with PC1 (the dominant embedding axis).
    ...
    for j, (k, lab) in enumerate(zip(keys, labels)):
        ...
        rows.append({"key": k, "label": lab, "meaning": _PARAM_MEANING.get(k, ""),
...
def _figure_collapse_tracking(pc1, theta, keys, ranked):
    """(a) |Spearman rho| of PC1 with each imaging parameter, sorted; (b) PC1 against the top one."""
    ...
    ax_b.set_title("(a) imaging parameter vs the dominant axis")
    top = ranked[0]
...
def _write_report(R, args, comps, exp, present, groups_emb, n_ev_tasks, pooled_kind, n_pooled_exp,
    ...
    if synth_theta is not None:
        ...
        if len(pc1_synth) == len(synth_theta):
            ranked = _track_collapse(pc1_synth, synth_theta, theta_keys, theta_labels)
            reporter.table(
                "PC1 vs imaging parameters (Spearman) — collapse driver",
...
def _run(args, R):
    ...
    if args.dry_run:
        ...
        print("[DRY RUN] Embedding-space distance — would read/write:")
        print(f"    estimator     : {R['estimator_path']}  [{'OK' if R['estimator_path'].exists() else 'MISSING'}]")
        print(f"    experimental  : {R['experiment_dir']}/Experiment_<KIND>_Cell_<n>_{args.experiment_span_seconds}S_RAW.tif")
        for k in kinds:
            print(f"        {k:<4}: {len(_discover_cells(R['experiment_dir'], k, args.experiment_span_seconds))} recording(s)")
        ...
        print(f"    window        : {n_frames} frames, step {step_frames}; mix={args.mix}, alpha={args.alpha}")
        print(f"    writes        : {R['posit_dir']}/{R['paths'].project_alias}_{R['timing_label']}_Embedding_Space_Distance/")
        return
    ...
    if not present:
        raise FileNotFoundError(
            ...
            f"at span {args.experiment_span_seconds}s. Stage the recordings first.")
    ...
    # Pooled experimental group (mix=natural keeps the dataset's proportions; balanced draws an equal
    # number of windows per condition). Each recording stays its own cell either way.
    ...
    if len(present) == 2:
        a, b = present
        comps[f"{a}_vs_{b}"] = _compare(exp[a]["emb"], exp[a]["cells"], exp[b]["emb"], exp[b]["cells"],
                                        alpha=args.alpha, n_perm=args.n_permutations, seed=0)

    # Secondary diagnostic matching the figure's L2 row: the pooled MMD in the L2-normalized (cosine)
    # geometry. Raw stays the geometry of record (the verdict table); this is a scale check only.
    ...
    if synth_theta is None:
        print("  (EVAL imaging theta not found — skipping PC1 parameter tracking)")
    ...
    print(f"\nprimary (pooled) verdict: {comps['pooled']['verdict']}  "
          f"(C2ST acc {comps['pooled']['c2st']['accuracy']:.3f}, MMD p {comps['pooled']['mmd']['p_value']:.3g})")
...
def parse_args(argv):
    p = argparse.ArgumentParser(
        description="Experimental-versus-synthetic embedding distance (Detector realism analysis).")
    ...
    p.add_argument("--kinds", type=str, default="ALP,BET",
                   help="comma-separated internal condition keys matching the recording filenames "
                        "(default 'ALP,BET'; shown as MET-FAB / MET-INLB in the figures and tables); "
                        "each is a diagnostic row, all are pooled for the primary row.")
    p.add_argument("--eval-tasks", type=int, default=None,
                   help="number of synthetic EVAL tasks to embed as the reference (default: all present).")
    ...
    p.add_argument("--max-cells", type=int, default=0,
                   help="cap the recordings per kind (0 = all; default 0).")
