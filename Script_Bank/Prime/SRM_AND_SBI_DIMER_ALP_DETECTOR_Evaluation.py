...
and the held-out ``_DETECTOR`` EVAL namespace (videos with known ground-truth
imaging theta), estimates the MAP parameter vector for each EVAL video via the
seed-then-optimize procedure in ``evaluation.map_estimate``, and reports how well
the inferred imaging parameters recover the truth through the same
``DiagnosticReporter`` report (per-parameter error table, inferred-vs-true and
residual figures, and — with ``--summary`` — posterior-calibration coverage).
...
leak-free by construction: the estimator has never seen these videos for
...
output is ``_DETECTOR``-namespaced.

Outputs (``_DETECTOR``-namespaced, under
``<data_bank>/<posit_subdir>/<project_alias>_{timing_label}_MAP_Recovery/``):
    report.md                    -- recovery report (per-parameter error table +
                                     legend + embedded figures).
    figures/recovery_<KEY>.png   -- combined per-parameter figure: inferred-vs-true
                                     and residual (View A) plus posterior credible
                                     intervals (View B, log10 units).
...
def write_recovery_outputs(reporter, args, eval_cfg, recovery_array_path: Path,
    ...
    headers, rows = recovery_table(det.DETECTOR_PARAMETERIZATION, true_log10, inferred_log10,
                                   guide, guide_tight)
    reporter.table(
    ...
    if post_q is not None:
        ...
        cov_headers, cov_rows = posterior_coverage_table(det.DETECTOR_PARAMETERIZATION, true_log10, post_q)
        reporter.table(
    ...
    if reporter.dump:
        for i, para in enumerate(det.DETECTOR_PARAMETERIZATION):
            key = para["KEY"]
...
def main(args: argparse.Namespace) -> None:
    ...
    paths = det.detector_paths(PARAMETERS.paths)   # Detector-aliased namespace
