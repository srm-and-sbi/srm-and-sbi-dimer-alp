"""Entry-point (Detector workflow): MAP-estimate imaging parameters from real videos.
...
``SRM_AND_SBI_DIMER_ALP_Experiment.py``: where Evaluation recovers parameters from
held-out *simulated* EVAL videos (known ground truth), this stage applies the same
seed-then-optimize ``map_estimate`` to *real* experimental videos, for which there
...
Detector differences: the estimator is loaded from a version-portable artifact
(``artifacts.load_estimator``) rather than a pickled posterior; the prior,
parameter table, and data paths are the Detector's (``det.build_prior``,
...
<data_bank>/<posit_subdir>/<project_alias>_{timing_label}_MAP_Experiment/):
...
def write_experiment_outputs(reporter, args, eval_cfg, array_path: Path,
    ...
    inferred_by_kind = _aggregate_by_kind(
        ...
        args.aggregation, len(det.DETECTOR_PARAMETERIZATION))
    ...
    headers, rows = experiment_table(det.DETECTOR_PARAMETERIZATION, inferred_by_kind, kinds)
