...
``PARAMETERS.simulation.timing.label`` to namespace files by duration + fps):
    ...
    <data_bank>/<video_subdir>/<alias>_DETECTOR_{timing_label}_Video_Set_TASK_{n}_{split}.zarr
        (or .npy if --no-compress)
    <data_bank>/<theta_subdir>/<alias>_DETECTOR_{timing_label}_Theta_Set_TASK_{n}_{split}.zarr
    ...
    for loop_i, task in enumerate(task_indices):
        ...
        if compress:
            ...
            theta_store = zarr.open(
                store=str(theta_set_path), mode="w", shape=theta_data.shape,
...
if __name__ == "__main__":
    ...
    # Pass the Detector-aliased Paths so a --debug-dump transcript lands in the
    # _DETECTOR-namespaced Debug dir (never colliding with the canonical DLI one),
    # matching the DiagnosticReporter dump_dir, which already uses the det paths.
