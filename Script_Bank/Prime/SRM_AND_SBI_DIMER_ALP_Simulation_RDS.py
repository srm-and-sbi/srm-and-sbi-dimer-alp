"""Entry-point script: generate ReaDDy reaction-diffusion trajectories.

Samples DIMER-model parameters from the log-uniform prior, runs ReaDDy
simulations for each (task, simulation) pair, and saves the resulting
particle trajectories as .h5 files and the parameter samples as a .zarr
(compressed) or .npy (uncompressed) theta set.
...
def main(args: argparse.Namespace) -> None:
    ...
    # coefficient (diffusivity_alp) plus the dimer diffusivities relative to it
    # (relative_diffusivity_bet/chi); and the reaction rates -- association
    # (relative_rate_dimerization, relative to the diffusion-limited cap),
    # dissociation (rate_dissociation), immobilization (rate_immobility), and
    # mobilization (rate_mobility). See parameterization.py for the full spec
    # (order, units, prior ranges).
    ...
    for loop_i, task in enumerate(task_indices):
        ...
        traj_dir = PARAMETERS.paths.trajectory_dir(task_alias, data_bank_root, timing_label, split)
        traj_dir.mkdir(parents=True, exist_ok=True)

        # Write the theta set: incremental zarr.open for compressed, save_theta_set for plain.
        ...
        for sim in range(args.task_simulations):
            ...
            stem = build_system(theta, verbose=args.verbose)
            ...
            # regardless; the sampled theta is persisted per task and the global
            # task index is the provenance handle.
            smut = build_simulation(stem, theta, seed=args.seed, verbose=args.verbose)
