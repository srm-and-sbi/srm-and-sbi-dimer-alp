...
**Deferred (flagged for a later iteration):** the brightness-flicker degeneracy and other imaging-parameter degeneracies (gain × conversion, offset × baseline, brightness scale × PSF width) call for a reparameterization rather than a wider box; the forward model has no stochastic EM-gain (gamma/Erlang) register — gain is a scalar multiply, with no excess-noise factor — a fidelity upgrade for a later iteration; and the readout-noise term scales a unit normal by `variance / gain^2` (a variance where a standard deviation belongs), giving a post-gain read-noise standard deviation of `variance / gain` instead of the physically standard, gain-independent `sqrt(variance)` — retained because the tuned model matches the real pixel-intensity distribution (see the read-noise note under DLI Imaging in `PROJECT_CONTEXT.md`), with the corrected form deferred to the same later iteration.
