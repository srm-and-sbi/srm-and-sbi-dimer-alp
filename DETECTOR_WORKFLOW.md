...
- **Overall system gain** `γ = g/C` (ADU per photoelectron) from the photon-transfer slope; a single photon-transfer curve identifies only this ratio, so the electron-multiplication gain `g` and the conversion factor `C` are anchored to the camera spec (backed by a gain-series if needed), not decomposed from the videos (see `REFERENCE_EMCCD_NOISE_MODEL.md` §6, §9).
...
Log-uniform priors; the prior center shown is the range's geometric midpoint (a representative value, not a fixed setting).

| parameter | log10 range | absolute | prior center | forward-model role |
|---|---|---|---|---|
| `kappa_c` | (0, 1) | 1–10 | 3.16 | photon→ADU conversion; scales offset and readout variance |
| `kappa_o` | (2, 3) | 100–1000 | 316 | camera offset/pedestal |
| `kappa_g` | (1.5, 2.5) | 31.6–316 | 100 | EM gain |
| `kappa_v` | (1, 2) | 10–100 | 31.6 | read-noise standard deviation |
| `mu_r` | (0, 0.5) | 1–3.16 | 1.78 | median PSF spread (log-normal scale) |
| `sigma_r` | (−1, 0) | 0.1–1 | 0.316 | emitter-to-emitter PSF variability (log-normal shape) |
| `mu_pc` | (1.75, 2.75) | 56.2–562 | 178 | median emitter brightness (log-normal scale) |
| `sigma_pc` | (−1, 0.25) | 0.1–1.78 | 0.422 | brightness spread (log-normal shape); also sets the flicker length scale `sigma_bright` (§6.3) |
| `prob_photo_bleach` | (−2, −0.5) | 0.01–0.316 | 0.056 | photobleaching probability over the reference frame count; sets the rate into the dark state |
| `lambda_rate` | (−1.25, 0.25) | 0.056–1.78 | 0.316 | brightness-switching rate — the inferred flicker parameter (§6.3) |

### 6.3 The brightness-flicker model: a derived locality, an inferred rate
...
`Q[i,j] = lambda_rate · exp( −|brightness_i − brightness_j| / (kappa_penalty · sigma_bright) )`,

where `sigma_bright = mu_pc · exp(sigma_pc²/2) · sqrt(exp(sigma_pc²) − 1)` is the photon-space standard deviation of the emitter-brightness log-normal and `kappa_penalty = 1` is a fixed dimensionless locality (the rate falls by one e-fold per one brightness standard deviation of change). Each non-dark state also has a rate into the absorbing photobleached state 0 (`epsilon = −ln(1 − prob_1) / delta_frame`, `prob_1 = 1 − (1 − prob_photo_bleach)^(1/numb_photo_bleach)`); the diagonal is `Q[i,i] = −Σ_j Q[i,j]`; and the per-frame transition matrix is `P = expm(Q · delta_frame)` (Norris 1997, *Markov Chains*, CUP, ch. 2, for the holding times, embedded jump chain, and matrix-exponential semigroup). At the prior-center brightness (`mu_pc ≈ 178`, `sigma_pc ≈ 0.422`, quantiles `[0, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95]`) the eight brightness states are `[0, 89, 104, 134, 178, 236, 305, 356]` photons and `sigma_bright ≈ 86`.

**Why the rate is inferred and the locality is fixed.** A flicker generator carrying both a free rate and a free locality is only weakly identifiable: at a fixed flicker level the rate scale and the locality trade off along a diagonal ridge (iso-flicker contours in the rate–locality plane), separated only by the far-jump probability — the chance a switch moves two or more brightness levels — a small, second-order signal further confounded by the camera noise. In the stiff/sloppy geometry of Transtrum et al. (2015, *J. Chem. Phys.* 143:010901) the switching rate (equivalently the mean dwell between switches) is the stiff, well-constrained direction and the locality is the sloppy one. The model therefore infers only the rate, `lambda_rate`, and fixes the locality by tying its length scale to `sigma_bright`. Fixing the sloppy direction while inferring the stiff one is the parameter-orthogonality prescription (Cox & Reid 1987, *JRSS-B* 49:1–39); measuring the brightness change in units of `sigma_bright` additionally decouples the flicker from the brightness magnitude — `mu_pc` cancels in `|Δb| / sigma_bright`, since both the level spacings and `sigma_bright` scale with it, leaving only a mild residual dependence on the shape `sigma_pc`.

**Flicker regime.** At `kappa_penalty = 1` and the prior-center brightness the length scale is `sigma_bright ≈ 86` (`gamma_eff = 1/sigma_bright ≈ 0.012`), and a switch moves two or more brightness levels about 46 % of the time — an erratic, large-step flicker; posterior-predictive inspection of rendered clips against the real recordings supports this large-step character over a gradual, neighbor-to-neighbor one. `lambda_rate` carries a log-uniform prior `(−1.25, 0.25)` (linear `0.056–1.78`); at the prior-center brightness this spans mean dwells of roughly 0.25–8 s and brackets the dwell implied by the real-data flicker (about 0.5–2.7 s, median ≈ 1 s).

**Imaging priors from the data.** The learnable imaging priors are anchored to the maximum-a-posteriori estimates on the real recordings (recovery is validated on the held-out synthetic EVAL namespace; the real recordings have no ground truth): the gain `kappa_g` prior `(1.5, 2.5)` brackets a real-data MAP near 87, the median brightness `mu_pc` prior `(1.75, 2.75)` a median near 120, and the brightness shape `sigma_pc` prior `(−1, 0.25)` a median near 0.71.
...
**Running the construction (`SRM_AND_SBI_DIMER_ALP_DETECTOR_Nuisance_DLI`).** The step is implemented as a two-mode script run once the Detector Experiment MAP output exists, keyed by recording duration (`--total-time-seconds`, which sets the timing label). *Emit-template mode* (the default) summarizes the pooled per-window MAP estimates against the imaging prior box and writes a value-based specification template (`..._Nuisance_DLI_Spec.toml`, every value in log10 and pre-filled as a suggestion) plus the pooled per-window MAP vector (`..._Nuisance_DLI_PosteriorSamples.npz`) as a ready candidate for the `samples` form. The person then edits the template to set each parameter's role and final values. *Build mode* validates the finalized specification against the prior box and builds the samplable `..._Nuisance_DLI.npz` artifact with its `parameter_keys` manifest; the `posterior` block form validates but defers its build until a trained imaging estimator conditioned on the real recordings exists, so the `fixed`, `uniform`, and `samples` forms are the buildable routes meanwhile. The printed MAP and 5th/95th-percentile spread are calibrated estimates on real microscopy recordings, which have no ground truth — decision support for a human choice, not a demonstrated recovery of the true imaging parameters (recovery is quantified only on held-out synthetic data in the Detector Evaluation stage, §9, B5). Usage detail — the arguments, the emitted summary, choosing a role per parameter, and the caveats — is in the companion note `SRM_AND_SBI_DIMER_ALP_DETECTOR_Nuisance_DLI.md` (`Script_Bank/Analysis`).
...
**Closed:** justification and reproducibility of every imaging parameter; a posterior summary (estimate plus credible interval) rather than a bare point; a prior-bounded (or explicitly logged out-of-bound) maximum-a-posteriori estimate; simulation-based calibration and coverage diagnostics (SBC, Talts et al. 2018; expected coverage and the trust crisis, Hermans et al. 2022; TARP, Lemos et al. 2023; local C2ST, Linhart et al. 2023); a quantitative real-versus-synthetic distance; disclosed training configuration; and the brightness-flicker identifiability, resolved by inferring only the switching rate and deriving the locality from the brightness scale (§6.3).

**Deferred (flagged for a later iteration):** the remaining imaging-parameter degeneracies (gain × conversion, offset × baseline, brightness scale × PSF width) call for a reparameterization rather than a wider box; the forward model has no stochastic EM-gain (gamma/Erlang) register — gain is a scalar multiply, with no excess-noise factor — a fidelity upgrade for a later iteration; and the readout-noise term, in the single function `add_noise`, scales a unit normal by `variance / gain^2` (a variance where a standard deviation belongs) and adds it before the gain rather than after the register, so the effective post-gain read-noise standard deviation is `variance / gain` instead of the physically standard, gain-independent `sqrt(variance)` — retained because the tuned model comes close to (though does not exactly reproduce) the real pixel-intensity distribution, with a candidate correction, not yet settled, left to a later iteration (see the read-noise note under DLI Imaging in `PROJECT_CONTEXT.md`). The corrected model these three items point toward — the stochastic Gamma electron-multiplication register that restores the excess-noise factor, the gain-independent read noise added after the register, and the unit-explicit `(quantum efficiency, EM gain, conversion factor, read noise, bias)` parameterization that separates the coupled gain, conversion, and read-noise scales — is specified in full, with its derivations, its background-illumination model, and its validation protocol, in `REFERENCE_EMCCD_NOISE_MODEL.md`.
...
**Deferred, separately and cautiously:** the canonical `parameterization.py` and its DLI stage require a rework before their next production run — carrying in the brightness-flicker generator's derived-locality form (§6.3; the shared `compute_matrices` now derives the locality from the brightness scale) and the revised diffusion and count ranges (§6.1), under a value-based-role parameterization that lets each parameter block be held fixed, inferred, or marginalized as a nuisance rather than fixed as a vector only. Until that rework the canonical DLI stage does not run — it still supplies the now-removed brightness-transition penalty (`gamma_penalty`) to the shared generator. This is a deliberate, separate step, not done here.
